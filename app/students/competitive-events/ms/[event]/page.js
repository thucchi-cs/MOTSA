...
export default async function HSEvents({ params }) {
  ...
  const info = JSON.parse(fileContents).find(group => group.lvl === "middle").events.find(event => event.id === id);
  
  // Get theme from json db
  const fileContents2 = await fs.readFile('./data/event_themes.json', 'utf8');
  const themeResults = JSON.parse(fileContents2).find(event => event.id === id);
